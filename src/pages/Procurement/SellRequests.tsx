...
import type { SellRequest } from "../../api/services/sell.service";
import { CONDITION_LABELS, SELL_STATUS_COLORS, SELL_STATUS_LABELS, money } from "./sellUi";

/**
 * Procurement queue for customer sell (trade-in) requests. Mirrors the Repair queue, but this
 * team's job is to price what Buyology PAYS, so the money column shows the offer.
 */
export default function SellRequests() {
  ...
  return (
    <>
      <PageMeta title="Sell requests | Buyology" description="Price customer trade-in requests" />
      <PageBreadcrumb pageTitle="Sell requests" />
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        {loading ? (
          ...
          <div className="rounded-xl border-2 border-dashed border-gray-200 p-8 text-center dark:border-gray-700">
            <p className="text-gray-500 dark:text-gray-400">No sell requests yet.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {requests.map((req) => (
                  <tr
                    key={req.id}
                    onClick={() => navigate(`/procurement/sell-requests/${req.id}`)}
                  ...
                  >
                    ...
                    <td className="py-3 pr-4 font-medium text-gray-800 dark:text-gray-200 break-words">
                      ...
                      <div className="text-xs text-gray-400">
                        {req.brand} · {req.model} · {CONDITION_LABELS[req.deviceCondition]}
                      </div>
                    </td>
                    ...
                    <td className="py-3 pr-4 text-gray-800 dark:text-gray-200">
                      {req.offerPrice != null ? (
                        money(req.offerPrice, req.offerPriceCurrency)
                      ) : req.aiEstimateMinPrice != null && req.aiEstimateMaxPrice != null ? (
                        <span className="text-xs text-violet-600 dark:text-violet-300">
                          AI {money(req.aiEstimateMinPrice, req.aiEstimateCurrency)} –{" "}
                          {money(req.aiEstimateMaxPrice, req.aiEstimateCurrency)}
                        </span>
                      ) : (
                        "—"
                      )}
                    ...
                    <td className="py-3 pr-4">
                      <span
                        className={`rounded-full px-2 py-0.5 text-xs font-medium ${SELL_STATUS_COLORS[req.status]}`}
                      >
                    ...
                    <td className="py-3">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          navigate(`/procurement/sell-requests/${req.id}`);
                        }}
